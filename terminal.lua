...
digipad.get_keyb_formspec = function(pos)  -- Construct updated formspec for keyboard
	local current_chan = minetest.get_meta(pos):get_string("chan_num")
	return "size[4,1;]"..
...
digipad.set_channel = function(pos, new_channel)
	minetest.get_meta(pos):set_string("channel", new_channel)
end
...
digipad.new_line = function(pos, text)
	...
	if lines > max_lines then  -- clear screen before printing the line - so it's never blank
		digipad.clear(pos)
		lines = 0 --update after clear
	end

	meta:set_string("formspec", meta:get_string("formspec")..
		"label[0," .. lines / 4 .. ";" .. string.sub(text, 1, max_chars) .. "]"
	)
...
minetest.register_node("digipad:keyb", {
	...
	on_receive_fields = function(pos, formname, fields, sender)
		...
		if fields.input then
			digiline:receptor_send(pos, digiline.rules.default, channel or meta:get_string("channel"), fields.input)
		end
...
minetest.register_node("digipad:terminal", {
	...
	on_receive_fields = function(pos, _, fields)
		...
		if string.sub(text,1,1) == "/" then  -- command is for terminal
			digipad.parse_cmd(pos, string.sub(text, 2))
			return
		end
		digiline:receptor_send(pos, digiline.rules.default, minetest.get_meta(pos):get_string("channel"), text)
